...
async def get_logged_user(token: Annotated[str, Depends(oauth2_scheme)]):
    ...
    for user in db: # procura o dicionário do usuário na base de dados baseado no username digitado
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    db = database.get("users")
    user = search_user(form_data.username, db) # procura um usuário com email igual ao do formulário
